...
- 输入序列重合:
  ...
  **以下是一个常用的列表**
  ...
  (这主要是来自以前为了输入控制字符的设计, 对于字母,
  ctrl 通常会将 ascii 码的高 3 位置 0, 即 `code&0b11111`)
...
- `C-w`: 向前删除一个词, 并压至yank栈顶
- `M-d`: 向后删除一个小粒度词, 并压至yank栈顶
- `C-u`, `C-k`: 删除至首部或尾部, 并压至yank栈顶
- `C-y`: 将yank栈中yank指针处的目标输入, 当栈变化时yank指针被重置到栈顶
...
> [!NOTE]
> 删除也被称作 kill, 粘贴操作也被称作 yank, 而一些 kill 操作会将删除的内容添加到yank栈栈顶
>
> 连续的yank栈压栈(指中间不含其它不压栈yank栈的操作), 会被拼接,
> 即 `abc def` 在末尾处连续按两次 `C-w` 并 `C-y` 会将整段文本粘贴, 而不是只有 `abc`
